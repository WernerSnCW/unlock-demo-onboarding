...
async function uploadPostcodeData2() {
  console.log('Starting additional postcode data upload...');
  
  const csvFilePath = './attached_assets/PostCodes_2_1756382800978.csv';
...
uploadPostcodeData2()
  .then(() => {
    console.log('Additional postcode data upload completed successfully');
    process.exit(0);
  })
  .catch((error) => {
    console.error('Upload failed:', error);
