...
describe('computeIncomeRunway', () => {
  it('survives when the buffer never runs dry within the window', () => {
    const result = computeIncomeRunway(replay({}), 10_000, 1_000_000, 'Test Episode');
  ...
  it('does not survive when the buffer runs dry before the recorded recovery', () => {
    const result = computeIncomeRunway(replay({}), 10_000, 15_000, 'Test Episode');
  ...
  it('does not survive when the episode never recovered in its window AND the buffer runs dry', () => {
    const result = computeIncomeRunway(replay({ recoverySteps: null }), 10_000, 15_000, 'Test Episode');
    expect(result.survivesWithoutSellingAtTrough).toBe(false);
  ...
  it('pro-rates monthly episodes by dividing annual spend by 12', () => {
    ...
    const result = computeIncomeRunway(monthlyReplay, 10_000, 2_000, 'Test Episode');
    expect(result.bufferExhaustedAtStep).toBe(3);
  });

  it('narrative distinguishes known-recovery exhaustion from never-recovered exhaustion', () => {
    const knownRecovery = computeIncomeRunway(replay({}), 10_000, 15_000, 'Test Episode');
    expect(knownRecovery.narrative).toContain('before the Test Episode recovery');
    expect(knownRecovery.narrative).not.toContain('never recovered');

    const neverRecovered = computeIncomeRunway(replay({ recoverySteps: null }), 10_000, 15_000, 'Test Episode');
    expect(neverRecovered.narrative).toContain('never recovered within its recorded window');
    expect(neverRecovered.narrative).not.toContain('before the Test Episode recovery');

    const survives = computeIncomeRunway(replay({}), 10_000, 1_000_000, 'Test Episode');
    expect(survives.narrative).toContain('would have covered essential spending');
  });
});
